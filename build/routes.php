...
foreach ( $routes_by_page as $page_slug => $page_routes ) {
	...
	$global_name = 'jpa_' . $page_slug_underscore . '_routes_data';
 ...
 */
function jpa_register_page_routes( $page_routes, $register_function_name ) {
	...
	foreach ( $page_routes as $route ) {
		...
		if ( $route['has_content'] ) {
			...
			if ( file_exists( $content_asset_path ) ) {
				...
				$content_handle = 'jetpack-premium-analytics/routes/' . $route['name'] . '/content';
		...
		if ( $route['has_route'] ) {
			...
			if ( file_exists( $route_asset_path ) ) {
				...
				$route_handle = 'jetpack-premium-analytics/routes/' . $route['name'] . '/route';
				$extension = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '.js' : '.min.js';
				// Deregister first to override any previously registered version
				// (e.g., Core's default modules when running as a plugin).
...
// Page-specific route registration functions for jetpack-premium-analytics
/**
 * Register routes for jetpack-premium-analytics page (full-page mode).
 */
function jpa_register_jetpack_premium_analytics_page_routes() {
	global $jpa_jetpack_premium_analytics_routes_data;
	jpa_register_page_routes( $jpa_jetpack_premium_analytics_routes_data, 'jpa_register_jetpack_premium_analytics_route' );
}
add_action( 'jetpack-premium-analytics_init', 'jpa_register_jetpack_premium_analytics_page_routes' );

/**
 * Register routes for jetpack-premium-analytics page (wp-admin mode).
 */
function jpa_register_jetpack_premium_analytics_wp_admin_page_routes() {
	global $jpa_jetpack_premium_analytics_routes_data;
	jpa_register_page_routes( $jpa_jetpack_premium_analytics_routes_data, 'jpa_register_jetpack_premium_analytics_wp_admin_route' );
}
add_action( 'jetpack-premium-analytics-wp-admin_init', 'jpa_register_jetpack_premium_analytics_wp_admin_page_routes' );
