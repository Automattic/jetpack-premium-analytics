...
global $jpa_jetpack_premium_analytics_routes, $jpa_jetpack_premium_analytics_menu_items;
$jpa_jetpack_premium_analytics_routes     = array();
$jpa_jetpack_premium_analytics_menu_items = array();

/**
 * Register a route for the jetpack-premium-analytics page.
 *
 ...
 */
function jpa_register_jetpack_premium_analytics_route( $path, $content_module = null, $route_module = null ) {
	global $jpa_jetpack_premium_analytics_routes;
	...
	$jpa_jetpack_premium_analytics_routes[] = $route;
}

/**
 * Register a menu item for the jetpack-premium-analytics page.
 *
 ...
 */
function jpa_register_jetpack_premium_analytics_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $jpa_jetpack_premium_analytics_menu_items;
	...
	$jpa_jetpack_premium_analytics_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the jetpack-premium-analytics page.
 *
 * @return array Array of route objects.
 */
function jpa_get_jetpack_premium_analytics_routes() {
	global $jpa_jetpack_premium_analytics_routes;
	return $jpa_jetpack_premium_analytics_routes ?? array();
}

/**
 * Get all registered menu items for the jetpack-premium-analytics page.
 *
 * @return array Array of menu item objects.
 */
function jpa_get_jetpack_premium_analytics_menu_items() {
	global $jpa_jetpack_premium_analytics_menu_items;
	return $jpa_jetpack_premium_analytics_menu_items ?? array();
}

/**
 * Preload REST API data for the jetpack-premium-analytics page.
 * Automatically called during page rendering.
 */
function jpa_jetpack_premium_analytics_preload_data() {
...
/**
 * Render the jetpack-premium-analytics page.
 * Call this function from add_menu_page or add_submenu_page.
 */
function jpa_jetpack_premium_analytics_render_page() {
	...
	do_action( 'jetpack-premium-analytics_init' );
	...
	jpa_jetpack_premium_analytics_preload_data();

	// Get all registered routes and menu items
	$menu_items = jpa_get_jetpack_premium_analytics_menu_items();
	$routes = jpa_get_jetpack_premium_analytics_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'jetpack-premium-analytics-prerequisites', '', $asset['dependencies'], $asset['version'], true );

		// Add inline script to initialize the app
		$init_modules = ["@jetpack-premium-analytics/init"];
		wp_add_inline_script(
			'jetpack-premium-analytics-prerequisites',
			sprintf(
				'import("@wordpress/boot").then(mod => mod.init({mountId: "%s", menuItems: %s, routes: %s, initModules: %s, dashboardLink: "%s"}));',
				'jetpack-premium-analytics-app',
		...
		wp_register_style( 'jetpack-premium-analytics-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for jetpack-premium-analytics module
		...
		// Add init modules as static dependencies
			$boot_dependencies[] = array( 'import' => 'static', 'id' => '@jetpack-premium-analytics/init' );
		 ...
		 * jetpack-premium-analytics page.
		 *
		...
		wp_enqueue_script( 'jetpack-premium-analytics-prerequisites' );
		wp_enqueue_script_module( 'jetpack-premium-analytics' );
		wp_enqueue_style( 'jetpack-premium-analytics-prerequisites' );
	}
	...
	$hook_suffix = 'jetpack-premium-analytics';
...
function jpa_jetpack_premium_analytics_intercept_render() {
	...
	if ( isset( $_GET['page'] ) && 'jetpack-premium-analytics' === $_GET['page'] ) {
		jpa_jetpack_premium_analytics_render_page();
		exit;
	}
}
