 ...
 *   jetpack-premium-analytics/widgets/{widget-dir-name}/render
 *   jetpack-premium-analytics/widgets/{widget-dir-name}/widget
 */
function jpa_register_widget_modules() {
	$widgets = jpa_get_registered_widget_modules();
	if ( empty( $widgets ) ) {
		return;
	}
...
if ( did_action( 'init' ) ) {
	jpa_register_widget_modules();
} else {
	add_action( 'init', 'jpa_register_widget_modules' );
}
		...
		$widgets = array_map(
			static function ( $widget ) {
				$widget['render_module'] = ! empty( $widget['has_render'] )
					? 'jetpack-premium-analytics/widgets/' . $widget['dir_name'] . '/render'
					: null;
				$widget['widget_module'] = ! empty( $widget['has_widget'] )
					? 'jetpack-premium-analytics/widgets/' . $widget['dir_name'] . '/widget'
					: null;
				return $widget;
			},
			$raw
